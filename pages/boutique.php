...
// Définition du titre par défaut de la page
$pageTitle = 'Boutique';
...
// Calcul de la page courante pour la pagination
...
// Construction du nom de la catégorie active pour l'affichage
$catNomActif = '';
if ($categorieId) {
    $ids = is_array($categorieArr) ? $categorieArr : [$categorieId];
    $noms = [];
    // Parcours de toutes les catégories pour trouver les noms correspondants
...
// Mise à jour du titre de la page avec le nom de la catégorie active
...
<div class="container" style="padding-top:16px;padding-bottom:48px;">
    <!-- Mise en page avec sidebar filtrée et grille produits -->
    <div class="boutique-layout">
        ...
        <aside class="sidebar">
            ...
            <?php // Formulaire de filtres envoyé en GET vers la même page ?>
            <form method="GET" action="<?= BASE_URL ?>/pages/boutique.php">
                <!-- Section filtre par catégorie -->
                <div class="sidebar-section">
                    <h4>Catégorie</h4>
                    <?php // Boucle sur chaque catégorie pour afficher une case à cocher ?>
                ...
                <!-- Section filtre par taille -->
                <div class="sidebar-section">
                    <h4>Taille</h4>
                    <?php // Boucle sur les tailles prédéfinies ?>
                ...
                <!-- Section filtre par note clients -->
                <div class="sidebar-section">
                    <h4>Note clients</h4>
        ...
        <main class="boutique-main">
            <?php // Vérification si des produits correspondent aux filtres ?>
            ...
            <div class="grid-3">
                <?php // Boucle sur chaque produit pour afficher sa carte ?>
                <?php foreach ($produits as $prod): ?>
                <div class="product-card">
                    <?php // Lien vers la page détail du produit ?>
                    <a href="<?= BASE_URL ?>/pages/detail_produit.php?id=<?= $prod['id'] ?>">
                        <div class="product-image-placeholder" style="position:relative;<?php if (!empty($prod['photo'])): ?>background-image:url('<?= UPLOADS_URL ?>/<?= $prod['photo'] ?>');background-size:cover;background-position:center;<?php endif; ?>">
                            ...
                            <?php // Indicateur de rupture de stock ?>
                    ...
                    <div class="product-info">
                        <?php // Détermination de la classe CSS du badge selon la catégorie ?>
                        <?php
                        ...
                        <?php // Tailles disponibles ?>
                        <?php if (!empty($prod['taille_disponible'])): ?>
                        <p class="taille"><?= securiser($prod['taille_disponible']) ?></p>
                        <?php endif; ?>
                        <?php // Prix du produit (avec solde éventuel) ?>
                        <div class="prix"><?= renderPrix($prod['prix'], $prod['solde_prix']) ?></div>
                        <?php // Affichage du stock disponible ?>
                        <?php if ($prod['stock'] > 0): ?>
                        <div class="stock-info" style="font-size:11px;margin-bottom:6px;">
                            <?php if ($prod['stock'] > 5): ?>
                            <span style="color:var(--success);"><i class="fas fa-check-circle"></i> En stock (<?= $prod['stock'] ?>)</span>
                            <?php else: ?>
                            <span style="color:var(--warning);"><i class="fas fa-exclamation-circle"></i> Stock limité (<?= $prod['stock'] ?>)</span>
                            <?php endif; ?>
                        </div>
                        <?php endif; ?>
                        <?php // Affichage du bouton d'ajout au panier si le stock est positif ?>
                        <?php if ($prod['stock'] > 0): ?>
            ...
            <?php if ($totalPages > 1): ?>
            <?php
            // Construction de la chaîne de requête pour conserver les filtres
            $qsParams = [];
            ...
            <div class="pagination">
                ...
                <?php // Construction de la liste des pages avec points de suspension ?>
                <?php
                $afficher = [];
                // Boucle pour générer la plage de pages à afficher
                for ($i = 1; $i <= $totalPages; $i++) {
                    // On affiche toujours la première, la dernière, et les pages autour de la page courante
                ...
                // Affichage de chaque élément de pagination
...
<!-- FOOTER MINI avec informations de la boutique -->
